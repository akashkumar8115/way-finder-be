...
def api_config():
    config = {
        "path": "",
        "status_code": 200,
        "tags": ["EmergencyService"],
        "summary": "Delete EmergencyService",
        "response_model": None,
...
async def delete_event_in_db(id: str):
    try:
        ...
        emergency_service = await EmergencyService.find_one(EmergencyService.emergency_service_uuid == uuid_id)
        if not emergency_service:
            raise HTTPException(status_code=404, detail="EmergencyService not found")

        await emergency_service.delete()

        return {"message": "EmergencyService deleted successfully", "event_id": id}
