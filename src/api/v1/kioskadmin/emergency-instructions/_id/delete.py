...
def api_config():
    config = {
        ...
        "summary": "Delete EmergencyInstruction encyService",
        "response_model": None,
...
async def delete_event_in_db(id: str):
    try:
        ...
        emergency_service = await EmergencyInstruction.find_one(EmergencyInstruction.id == id)
        if not emergency_service:
            raise HTTPException(status_code=404, detail="EmergencyInstruction not found")

        await emergency_service.delete()

        return {"message": "EmergencyInstruction deleted successfully", "event_id": id}
