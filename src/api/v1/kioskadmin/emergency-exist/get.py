...
async def main(
    ...
    try:
        ...
        emregency_service: List[EmergencyExit] = (
            await EmergencyExit.find({"entity_uuid": entity_uuid}).to_list()
        )

        if not emregency_service:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"No EmergencyExit found for entity_uuid: {entity_uuid}"
            ) 
