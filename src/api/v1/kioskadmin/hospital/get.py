...
def api_config():
    config = {
        "path": "",
        "status_code": 200,
        "tags": ["HospitalService Service"],
        "summary": "Get HospitalService Services",
        "response_model": dict,
        "description": "Retrieve all HospitalService services for an entity.",
        "response_description": "List of DocHospitalService  services",
        "deprecated": False,
    }
...
async def main(
    ...
    try:
        ...
        emregency_service: List[HospitalService] = (
            await HospitalService.find({"entity_uuid": entity_uuid}).to_list()
        )

        if not emregency_service:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"No HospitalService found for entity_uuid: {entity_uuid}"
            )  
