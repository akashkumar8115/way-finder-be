...
def api_config():
    config = {
        ...
        "status_code": 200,
        "tags": ["Doctor"],
        "summary": "Delete HospitalService encyService",
        "response_model": None,
...
async def delete_event_in_db(id: str):
    try:
        ...
        emergency_service = await HospitalService.find_one(HospitalService.id == id)
        if not emergency_service:
            raise HTTPException(status_code=404, detail="Doctor not found")

        await emergency_service.delete()

        return {"message": "HospitalService deleted successfully", "event_id": id}
