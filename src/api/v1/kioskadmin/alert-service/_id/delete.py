...
async def delete_event_in_db(id: str):
    try:
        ...
        emergency_service = await EmergencyAlert.find_one(EmergencyAlert.alert_id == uuid_id)
        if not emergency_service:
            raise HTTPException(status_code=404, detail="EmergencyAlert not found")

        await emergency_service.delete()

        return {"message": "EmergencyAlert deleted successfully", "event_id": id}
