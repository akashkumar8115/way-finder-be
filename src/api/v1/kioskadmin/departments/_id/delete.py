...
def api_config():
    config = {
        ...
        "status_code": 200,
        "tags": ["Doctor"],
        "summary": "Delete Department encyService",
        "response_model": None,
...
async def delete_event_in_db(id: str):
    try:
        ...
        emergency_service = await Department.find_one(Department.id == id)
        if not emergency_service:
            raise HTTPException(status_code=404, detail="Department not found")

        await emergency_service.delete()

        return {"message": "Department deleted successfully", "event_id": id}
