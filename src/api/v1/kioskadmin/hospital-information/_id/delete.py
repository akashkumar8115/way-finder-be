...
def api_config():
    config = {
        ...
        "summary": "Delete HospitalInformation encyService",
        "response_model": None,
...
async def delete_event_in_db(id: str):
    try:
        ...
        emergency_service = await HospitalInformation.find_one(HospitalInformation.id == id)
        if not emergency_service:
            raise HTTPException(status_code=404, detail="Doctor not found")

        await emergency_service.delete()

        return {"message": "HospitalInformation deleted successfully", "event_id": id}
