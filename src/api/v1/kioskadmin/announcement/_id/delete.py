...
def api_config():
    config = {
        ...
        "summary": "Delete DailyAnnouncement encyService",
        "response_model": None,
...
async def delete_event_in_db(id: str):
    try:
        ...
        emergency_service = await DailyAnnouncement.find_one(DailyAnnouncement.id == id)
        if not emergency_service:
            raise HTTPException(status_code=404, detail="Doctor not found")

        await emergency_service.delete()

        return {"message": "DailyAnnouncement deleted successfully", "event_id": id}
